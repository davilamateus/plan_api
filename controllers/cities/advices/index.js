...
const citiesAdvices = async (req, res) => {
    ...
    if (lat && lon && category) {
        ...
        const places = await axios.get(`https://api.foursquare.com/v3/places/search?local=${lat},${lon}${category ? `&categories=${category}` : ''}`, options)
            .then((data) => {
        ...
        const placesWithImages = await Promise.all(places.map(async (place) => {
            return imageResponse = await axios.get(`https://api.foursquare.com/v3/places/${place.fsq_id}/photos`, options)
                .then((images) => {
