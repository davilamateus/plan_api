...
router.get('/noticies/articles', (req, res) => {
    ...
    const response = async (key) => await axios.get(`https://newsdata.io/api/1/news?apikey=${key}&country=${country}&image=1&${category !== undefined ? 'category=' + category : ''}${page !== undefined ? 'page=' + page : ''}`)
        .then((data) => {
            res.json(data.data)
        }).catch((error) => {
            console.log('pub_31474e760b76a911c5b146e6ecab83971a987')
            response()
            res.json(error)
        })
    response('pub_31510e3fa57fdd9eaf76f8c8aad5b943efa97');
});
